...
df = pd.read_csv("../../data/processed/plays.csv")
...
model = NCF(
    num_pitchers, num_batters, num_outcomes, embed_dim=EMBED_DIM, num_layers=NUM_LAYERS
)
...
np.save("pitcher_encoder.npy", pitcher_encoder.classes_)
np.save("batter_encoder.npy", batter_encoder.classes_)
np.save("outcome_encoder.npy", outcome_encoder.classes_)
...
def evaluate_model(test_loader, model):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            pitcher_ids = pitcher_ids.to(device)
            batter_ids = batter_ids.to(device)
    ...
    )  # Replace target_names with your actual classes
