...
def analyze_outcomes(games):
    ...
    outcome_probabilities = {outcome: count / total_plays for outcome, count in outcome_counter.items()}
    
    # Sanity Checks
    sanity_checks = {
        'prob_sum': round(sum(outcome_probabilities.values()), 2) == 1.0,
        'missing_home_team_or_date': any('home_team' not in game or 'date' not in game for game in games),
    ...
    for game in games:
        for play in game.get('plays', []):
            if play['outcome'] == 'unknown':
                ...
                unknown_codes[play.get('raw_outcome', 'N/A')] += 1  # 'N/A' indicates that the raw outcome is missing
